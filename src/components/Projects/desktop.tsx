    ...
    <div className="mx-30 flex flex-col gap-5">
      ...
      <div className="grid grid-cols-4">
        <div className="w-140 h-120 bg-white rounded-2xl col-span-2">
          <div><img src="./mintr.png" alt="project" className="w-full h-70 rounded-t-2xl"/></div>
          <div className="p-5 flex flex-col gap-2">
            <div className="flex gap-2">
              <span className="bg-gray-300 text-primary-foreground p-2 text-xs rounded-3xl">Springboot</span> 
              <span className="bg-gray-300 text-primary-foreground p-2 text-xs rounded-3xl">React</span>
              <span className="bg-gray-300 text-primary-foreground p-2 text-xs rounded-3xl">PostgreSQL</span>
            </div>
            <p className="text-2xl font-bold">URL Shortener</p>
            <p className="text-sm text-primary">A URL shortening web app built with React and Spring Boot. The backend manages link generation and persistence, while the frontend provides a simple interface for creating and copying shortened URLs. </p>
          </div>
        </div>
            ...
            projects.map((project) => {
              return (
                <div className="w-80 h-120 bg-white rounded-2xl overflow-hidden">
                  <div><img src={project.image}  alt="project" className="w-full h-70 rounded-t-2xl object-cover"/></div>
                  <div className="p-5 flex flex-col gap-2">
                    <div className="flex gap-2"> 
                      {
